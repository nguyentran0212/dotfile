# ------------------------------------------------------------
# 1️⃣  Runtime image (multi-arch – no AUR, direct Ubuntu base)
# ------------------------------------------------------------
FROM ubuntu:25.04
...
RUN /bin/bash -c "source /usr/share/nvm/nvm.sh && nvm use node && NODE_VER=$(nvm current) && NODE_PATH=\"${HOME}/.nvm/versions/node/${NODE_VER}/bin\" && echo \"export PATH=\\\"${PATH}:$NODE_PATH\\\"\" >> ~/.zprofile"
